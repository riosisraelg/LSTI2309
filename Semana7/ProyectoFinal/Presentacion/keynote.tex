  ...
  \begin{columns}
    \column{.5\textwidth}
      ...
      \begin{itemize}
        ...
        VIF = 14.89 (crítico)
        \item[\alert{$\times$}] \texttt{bathrooms} \\
    ...
    \column{.5\textwidth}
      ...
      \begin{itemize}
        \item[\textcolor{green}{$\checkmark$}] \texttt{accommodates, bedrooms} \\
...
\begin{frame}{4. Análisis de Correlación}
  ...
  \begin{itemize}
    ...
    \item \textbf{bedrooms:} +0.293 (moderada)
    \item \textbf{bathrooms:} +0.155 (débil - excluida)
  \end{itemize}
\end{frame}
...
\begin{frame}{6. Construcción y Entrenamiento del Modelo}
  ...
  \begin{block}{Ecuación del Modelo}
    \begin{equation*}
      \text{log\_price} = \beta_0 + \beta_1 \cdot \text{acc} + \beta_2 \cdot \text{bedr} + \beta_3 \cdot \text{amen} + \beta_4 \cdot \text{clean} + \sum \text{dummies}
    \end{equation*}
  \end{block}
  ...
  \begin{itemize}
    ...
    \item \textbf{Variables:} 4 numéricas + room\_type + property\_type + bed\_type + neighbourhood = >650 predictores
